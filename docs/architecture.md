...
### Curve25519 module (2 pages, 128 KB)
...
Mutable total: 3740 bytes (4096..7836). `BUFFER_END = 7836` (< 65536 = 1 page); the module is sized at 2 pages so the TypeScript layer claims offsets 8192 through 131072 as the I/O staging region for caller-supplied inputs (seed, sk, pk, message, sig, digest, ctx) and outputs (pk, sig, shared secret). The WASM never reads or writes that region; the wrapper owns it and wipes it explicitly. `wipeBuffers()` zeroes from `MUTABLE_START` to `BUFFER_END` in one pass, covering every mutable region above; the AS data segment is read-only and is not wiped.
