...
`leviathan-crypto` is a strictly-typed cryptography library for the web. Zero dependencies, tree-shakable, side-effect free, with SIMD WASM at its core.

**JS is the problem, SIMD WASM is the solution.** JavaScript engines offer no formal constant-time guarantees. JIT compilers optimize based on runtime patterns, and those patterns leak secrets through cache access and instruction timing. By contrast, WebAssembly executes outside the JIT entirely, running compiled bytecode with linear memory you control. The same instructions run every time with no speculative optimization or value-dependent branches between source and execution.
...
_The paranoia of Serpent-256 and the elegance of XChaCha20-Poly1305, agreeing only on security properties._

**Serpent-256.** 32 rounds of S-boxes in pure Boolean logic, no table lookups. Every bit in every block goes through every round.

**XChaCha20-Poly1305.** 20 rounds of add-rotate-XOR, no S-boxes, no cache-timing leakage. Poly1305 adds an unconditional forgery bound.

Cipher choice is pluggable. The two ciphers share the same `CipherSuite` interface and slot into `Seal`, `SealStream`, and `SealStreamPool` interchangeably. Post-quantum coverage extends the same model. `KyberSuite` wraps ML-KEM-512, ML-KEM-768, or ML-KEM-1024 around any cipher suite, and the SPQR ratchet builds forward-secret sessions on top.
...
|`ratchet`|`ratchetInit`, `KDFChain`, `kemRatchetEncap`, `kemRatchetDecap`: SPQR KDF primitives (DR spec §5 + §7.2). Requires `sha2`; KEM ops also require `kyber` + `sha3`.|
|`stream`|`SealStream`, `OpenStream` (cipher-agnostic STREAM construction), `SealStreamPool` (worker-based parallelism)|

Pure TypeScript utilities (encoding helpers, random generation, format converters) ship alongside the WASM-backed primitives with no `init()` dependency.

### Auxiliary tier (not part of `Module` union)

- **`Fortuna`:** CSPRNG. Takes a `Generator` (cipher PRF) and a `HashFn` (stateless hash) at create time. Both ship from each cipher and hash module. See [fortuna.md](./fortuna.md) for valid combinations.
...
`test/` holds three independent categories of tests, run by separate CI workflows.
...
**Test vectors** (`vectors/`) is the immutable known-answer-test corpus. Files are read-only reference data. Some come from authoritative specifications (FIPS, RFCs, ACVP, NIST CAVP); others are generated locally as regression vectors by `scripts/gen-*-vectors.ts`. CI validates integrity against `SHA256SUMS`. See [test-suite.md](./test-suite.md) for the full corpus inventory and gate discipline.
...
**Build artifacts** (gitignored; only exist after `bun run build`). `build/` holds the raw `.wasm` outputs from AssemblyScript compilation. `dist/` is the published npm package contents (compiled JS, declarations, copied WASM, embedded blobs, doc subset). `CLAUDE.md` at the root is a build-time copy of `docs/CLAUDE_consumer.md` so AI assistants pulling the npm package have a project guide. `leviathan-crypto-<version>.tgz` is the `npm pack` output uploaded by the publish workflow.
...
<img src="https://github.com/xero/leviathan-crypto/raw/main/docs/arch-layers.svg" alt="Typescript Over Wasm Layered Diagram" width="800">
...
`SealStream`, `OpenStream`, and `SealStreamPool` are cipher-agnostic; you
select the cipher by passing `XChaCha20Cipher` or `SerpentCipher` at
construction.
...
<img src="https://github.com/xero/leviathan-crypto/raw/main/docs/build-pipeline.svg" alt="Build Pipeline Diagram" width="800">
...
<img src="https://github.com/xero/leviathan-crypto/raw/main/docs/import-graph.svg" alt="TS layer: internal import graph" width="800">
...
|`Fortuna`|`Generator` + `HashFn` (any compatible pair)|
...
The test vector corpus in `test/vectors/` is read-only. Integrity is verified via `SHA256SUMS`, expected values are sourced directly from authoritative references. They are the immutable truth, and must never be modified to make tests pass.
