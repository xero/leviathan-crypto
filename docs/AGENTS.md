...
- **Classical+PQ hybrid suite factory pattern** (`src/ts/sign/suites/hybrid-classical.ts`,
  format bytes `0x20`-`0x23`): implements `draft-ietf-lamps-pq-composite-sigs`
  Composite ML-DSA. Differs from the PQ-only hybrid pattern in several
  ways, each of which is spec-driven and must not be "harmonised" toward
  the PQ-only shape:
  - **ctx binding is via M', not `buildEffectiveCtx`.** Per composite-sigs
    §3.2 step 2, M' = `Prefix || Label || len(ctx) || ctx || PH(M)`. The
    32-byte ASCII Prefix `CompositeAlgorithmSignatures2025` and the
    per-suite Label come from composite-sigs §2.2 / §6 and are wedged
    into M' verbatim. Wrapping user_ctx with leviathan's
    `{ctxDomain}|{user_ctx}` framing inside M' would produce a wire
    incompatible with every other Composite ML-DSA implementation.
    `ctxDomain` is still present on the suite for catalog symmetry but
    never on the call path. Per-call cap is 255 bytes inline per
    composite-sigs §3.2 step 1, same `sig-ctx-too-long` discriminator.
  - **ML-DSA composite sk is seed-only (32 bytes).** Per composite-sigs
    §4.2, the wire sk is `mldsaSeed (32) || tradSk_encoded`. Every
    composite sign call re-derives the expanded ML-DSA sk via
    `MlDsaXX.keygenDerand(seed)` (FIPS 204 §6.1 KeyGen_internal); the
    expanded sk lives in WASM scratch for the duration of one sign and
    is wiped on exit. Slicing is `sk.subarray(0, 32)`, NOT
    `sk.subarray(0, mldsaParams.skBytes)`; the latter would read past
    the composite sk's actual layout. Standalone ML-DSA suites continue
    to hold and use expanded sks directly. The two patterns coexist.
  - **ML-DSA native ctx is the Label bytes.** Per composite-sigs §3.2
    step 4, the underlying ML-DSA sub-signer is invoked with
    `mldsa_ctx = Label`, not with user_ctx and not empty. The Label
    bytes flow through ML-DSA's pure-mode ctx parameter (FIPS 204 §5.2
    Algorithm 2), wrapping as `0x00 || |Label| || Label || M'` at the
    primitive layer.
  - **Pure ML-DSA, NOT HashML-DSA.** Composite-sigs §2.1 mandates pure
    ML-DSA inside the composite even though the composite itself
    pre-hashes M. Use `MlDsaXX.sign(sk, M', ctx=Label)`, not
    `signHashPrehashed` / `verifyHashPrehashed`.
  - **ECDSA half hashes M' with SHA-256 in both ECDSA suites.** Per
    composite-sigs §6, both `id-MLDSA44-ECDSA-P256-SHA256` and
    `id-MLDSA65-ECDSA-P256-SHA512` use `ecdsa-with-SHA256` as the
    traditional signature algorithm. The composite PH (SHA-256 at 0x22,
    SHA-512 at 0x23) determines the PH(M) inside M'; it does NOT change
    the ECDSA-internal hash, which is always SHA-256(M').
  - **Component encodings on the wire** (composite-sigs §4): ECDSA pk is
    65-byte uncompressed SEC 1 §2.3.3 (`0x04 || X || Y`), NOT the
    33-byte compressed form standalone `EcdsaP256Suite` uses; the
    composite calls `EcdsaP256.keygenUncompressed()` for keygen and
    `pointDecompress(pk33)` to expand a compressed pk on ingress paths.
    ECDSA sk on the wire is RFC 5915 §3 DER `ECPrivateKey` (51 bytes),
    encoded / decoded via `encodeEcPrivateKey` / `decodeEcPrivateKey`
    from `src/ts/ecdsa/ecprivatekey-der.ts`. ECDSA sig on the wire is
    RFC 3279 §2.2.3 DER `Ecdsa-Sig-Value` (variable, ≤ 72 bytes), NOT
    the raw r‖s form standalone `EcdsaP256Suite` uses. Ed25519 pk / sk
    / sig are raw bytes as in the standalone Ed25519 suite.
  - **ECDSA verify normalises high-S to low-S before delegation.** The
    composite verify path calls `normaliseEcdsaSigLowS` (s ← n - s when
    s > n/2) before handing the bytes to `EcdsaP256.verify`. This
    matches Appendix E reference vectors, which include high-S cases
    that the standalone strict-S `EcdsaP256.verify` would reject. The
    emit path stays strict (the WASM-side `ecdsaSign` already enforces
    low-S per FIPS 186-5 §6.5 / RFC 6979 §3.5). The asymmetry is
    intentional; see SECURITY.md §"ECDSA-half high-S normalisation" for
    the sEUF-CMA implication. Do NOT remove the normalisation step in a
    "matches the standalone suite" refactor.
  - **Variable-length ECDSA sigs are real.** `sigMaxSize` on the
    composite ECDSA suites is the catalog-reserved upper bound from
    composite-sigs Appendix A Table 4. The actual sig length floats
    under the DER encoding rules; the envelope wire's `payload_len`
    field at the head (see "Sign envelope wire format" below) lets sig
    fill the remainder.
- **Sign envelope wire format**: the attached envelope (`Sign.sign`,
  `SignStream.finalize` blob, `VerifyStream` input) carries the v3 shape
  `[suite_byte:u8][ctx_len:u8][ctx][payload_len:u32 BE][payload][sig]`.
  Sig fills the remainder; for fixed-length suites the trailing sig
  matches `suite.sigMaxSize` exactly, and for variable-length suites
  (composite ECDSA at `0x22` / `0x23`) the sig length floats up to
  `sigMaxSize`. The `payload_len` field is the load-bearing departure
  from the earlier "deduce payload from total length and known sig
  length" shape; do NOT regress to that pattern even on fixed-length
  suites. `Sign.peek` returns the same `{ suiteByte, ctx, payloadOffset,
  payloadLength, sigOffset }` shape it always did; only the internal
  derivation reads `payload_len` from the wire. `Sign.signDetached` and
  `Sign.verifyDetached` remain envelope-free and return / accept the
  suite's natural sig bytes for interop.
- **`SignatureSuite.sigMaxSize` (upper bound, not exact)**: the suite
  interface exposes `sigMaxSize`, not `sigSize`. For fixed-length
  signature schemes the max equals the actual size. For variable-length
  schemes (composite ECDSA) the max is the catalog-reserved upper
  bound; the actual sig may be shorter. Envelope receivers can
  pre-allocate buffers from `sigMaxSize` without knowing whether the
  suite is fixed or variable. Do NOT introduce a parallel `sigSize`
  field; the rename is intentional and any code path that wants to
  assert "this sig is exactly N bytes" should pin it from the suite's
  own constants, not from the interface.
...
- **ctxDomain construction**: every SignatureSuite carries a built-in
  ...
  Per-call user_ctx ≤ 255 bytes absolute (FIPS 204 §3.6.1 native ctx cap),
  throws `SigningError('sig-ctx-too-long')`. `buildEffectiveCtx` runs a
  second check that the combined effective_ctx (`1 + len(ctxDomain) + 1 +
  len(user_ctx)`) also fits 255 bytes; the per-call ceiling on
  `buildEffectiveCtx`-using suites is therefore `253 - len(ctxDomain)`.
  Both throws share the `sig-ctx-too-long` discriminator. The composite
  classical+PQ hybrids (`0x20`-`0x23`) bypass `buildEffectiveCtx` entirely
  and enforce the full 255-byte cap inline; their ctx binding lives in the
  M' construction, not in the effective_ctx wrapping.
