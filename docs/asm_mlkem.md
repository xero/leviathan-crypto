...
### `mlkem` ML-KEM WASM Module Reference

This low-level reference details the ML-KEM AssemblyScript source and WASM exports, intended for those auditing, contributing to, or building against the raw module. **Most consumers should instead use the [TypeScript wrapper](./mlkem.md) or the higher-level [AEAD classes](./aead.md#mlkemsuite).**
...
underlying scheme is ML-KEM: a structured-lattice KEM based on the hardness of
...
**k-parameterized design.** A single `mlkem.wasm` serves all three parameter
...
See [ML-KEM implementation audit](./mlkem_audit.md) for algorithm correctness verifications.
...
All exported functions are re-exported through `src/asm/mlkem/index.ts`. The
...
The mlkem WASM module has no explicit error return codes. Every function
...
| [mlkem](./mlkem.md) | TypeScript wrapper classes (`MlKem512`, `MlKem768`, `MlKem1024`, `MlKemSuite`) |
