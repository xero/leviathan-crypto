...
await init({ ed25519: curve25519Wasm })
```
...
await ed25519Init(curve25519Wasm)
```
...
const sig    = ed.signPrehashed(secretKey, publicKey, digest)
```

The caller supplies the 64-byte SHA-512 digest; the library does
not compute it. Wrong-length digest throws `RangeError`. `ctx`
defaults to an empty `Uint8Array`. Per RFC 8032 §5.1, Ed25519,
`|C|` is encoded in a single octet, so 255 is the spec ceiling;
longer ctx values throw `RangeError`. The dom2(F=1, ctx) prefix
binds ctx into both SHA-512 inputs that produce r and k.
...
| `RangeError: leviathan-crypto: ed25519 message length N exceeds the per-call ...`      | Message larger than the WASM input scratch.                      |
