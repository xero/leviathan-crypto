...
One-shot authenticated encryption and decryption. `Seal` is a static class —
never instantiated. Produces a blob that is a valid single-chunk `SealStream`
output: `preamble || finalChunk(counter=0, TAG_FINAL)`. `OpenStream.finalize()`
can decrypt a `Seal` blob directly.
...
**Constructor:** `Seal` has no constructor. All methods are static.
...
**`opts.aad`** — optional `Uint8Array`. Additional Authenticated Data: authenticated
but not encrypted. Same value must be passed to both `encrypt` and `decrypt`.

**Compatibility:** A `Seal` blob is structurally identical to a single-chunk
`SealStream` output. `OpenStream.finalize(blob.subarray(preambleLen))` will
decrypt it. The inverse is also true: a single-chunk `SealStream` can be
opened with `Seal.decrypt`.
...
const preamble = sealer.preamble;       // Uint8Array — send first (20B symmetric, 20B+kemCt for KEM)
...
**Throws if the preamble's format enum doesn't match the cipher, or if the preamble is too short.**
...
| `header`           | The 20-byte stream header. Note: `SealStreamPool` exposes `.header` (symmetric streams only); `SealStream` exposes `.preamble` (supports KEM preambles). |
...
Factory function that wraps a `MlKemBase` instance and an inner `CipherSuite`
into a unified hybrid KEM+AEAD suite. The returned object implements `CipherSuite`
and adds a `keygen()` method.
...
// Sender (has encapsulationKey only)
...
Works identically with `SealStream` / `OpenStream`:

```typescript
// Sender
const sealer   = new SealStream(suite, ek)
const preamble = sealer.preamble    // 20B header + KEM ciphertext
...
**Preamble sizes by parameter set:**

| KEM | KEM ciphertext | Preamble total |
|-----|---------------|----------------|
| ML-KEM-512  | 768 bytes  | 788 bytes  |
| ML-KEM-768  | 1088 bytes | 1108 bytes |
| ML-KEM-1024 | 1568 bytes | 1588 bytes |

**Key derivation:** On encrypt, `kem.encapsulate(ek)` produces `sharedSecret`
and `kemCt`. HKDF is called with `info = encode(hkdfInfo) || kemCt` — the KEM
ciphertext is bound into the key derivation. On decrypt, `kem.decapsulate(dk, kemCt)`
recovers `sharedSecret`; the same HKDF with the same `kemCt` is applied. The
inner cipher's `deriveKeys` runs on the HKDF output.

`KyberSuite` requires both `kyber` and `sha3` (for ML-KEM) plus the inner
cipher's modules (`chacha20` + `sha2` for `XChaCha20Cipher`).
