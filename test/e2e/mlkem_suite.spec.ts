...
test.describe('MlKemSuite, e2e (MlKem768 + XChaCha20)', () => {
	test('Seal one-shot round-trip (encrypt with ek, decrypt with dk)', async ({ page }) => {
		const result = await page.evaluate(async (base) => {
			...
			await lib.init({ chacha20: chacha20Wasm, sha2: sha2Wasm, sha3: sha3Wasm, mlkem: mlkemWasm });
			const kem = new lib.MlKem768();
			const suite = lib.MlKemSuite(kem, lib.XChaCha20Cipher);
