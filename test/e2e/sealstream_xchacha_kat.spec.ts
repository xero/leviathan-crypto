...
//
/**
 * SealStream XChaCha20 v3 Known-Answer Tests (cross-browser)
 *
 * Source: pinned vectors generated by scripts/gen-sealstream-vectors.ts
 *         (--cipher xchacha); each vector independently verified against
 *         raw primitives at generation time
 * Files:  vectors/sealstream_xchacha_v3.ts (xc1, xc3, xcf1)
 */
...
const xchachaVectors: SealStreamXChachaV3Vector[] = [xc1, xc3, xcf1];
...
test('SealStream KAT batch, XChaCha20 v3 (XC1, XC3, XCF1)', async ({ page }) => {
	const errors = await page.evaluate(async ({ base, vectors }) => {
		...
		const cipher = lib.XChaCha20Cipher;
		...
		return errors;
	}, { base: BASE, vectors: xchachaVectors });
