...
describe('XChaCha20Seal — wire format', () => {
	...
	it('first 24 bytes of ciphertext are the nonce', () => {
		...
		const seal  = new XChaCha20Seal(key);
		const ct    = seal._encryptWithNonce(new Uint8Array([0x01, 0x02]), new Uint8Array(0), nonce);
...
describe('XChaCha20Seal — hidden nonce validation', () => {
	it('wrong _nonce length throws RangeError', () => {
		...
		expect(() => seal._encryptWithNonce(pt, new Uint8Array(0), badNonce))
...
describe('XChaCha20Seal — key isolation', () => {
	it('different keys produce different ciphertext', () => {
		...
		const ct1   = seal1._encryptWithNonce(pt, new Uint8Array(0), nonce);
		const ct2   = seal2._encryptWithNonce(pt, new Uint8Array(0), nonce);
