...
//                           ▀█████▀▀
//
// test/unit/mlkem/helpers.ts
//
// WASM test helpers for the mlkem module. Loads build/mlkem.wasm directly
// (no init() system, mlkem WASM has its own memory, not imported).
...
let _instance: MlKemExports | null = null;

export async function loadMlKem(): Promise<MlKemExports> {
	if (_instance) return _instance;
	const wasmPath = join(__dirname, '../../../build/mlkem.wasm');
	const bytes = readFileSync(wasmPath);
	const { instance } = await WebAssembly.instantiate(bytes, {});
	_instance = instance.exports as unknown as MlKemExports;
	return _instance;
}

export function getWasm(): MlKemExports {
	if (!_instance) throw new Error('mlkem WASM not loaded, call loadMlKem() in beforeAll');
	return _instance;
}
