 ...
 * Files:  vectors/mlkem.ts (keygen, encap, decap_val, key_check vectors for 512/768/1024)
 */
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { join, dirname } from 'node:path';
import { describe, it, beforeAll, expect } from 'vitest';
import { loadMlKem, loadSha3, fromHex, toHex } from './helpers.js';
import { indcpaKeypairDerand, sha3_256Hash } from '../../../src/ts/mlkem/indcpa.js';
import { kemKeypairDerand, kemEncapsulateDerand, kemDecapsulate } from '../../../src/ts/mlkem/kem.js';
import { checkEncapsulationKey, checkDecapsulationKey } from '../../../src/ts/mlkem/validate.js';
import { MLKEM512, MLKEM768, MLKEM1024 } from '../../../src/ts/mlkem/params.js';
...
} from '../../vectors/mlkem.js';
import type { MlKemExports, Sha3Exports } from '../../../src/ts/mlkem/types.js';
import type { MlKemParams } from '../../../src/ts/mlkem/params.js';
...
describe('Gate 0, init system wiring', () => {
	beforeAll(async () => {
		_resetForTesting();
		const mlkemBytes = readFileSync(join(__dirname, '../../../build/mlkem.wasm'));
		const sha3Bytes  = readFileSync(join(__dirname, '../../../build/sha3.wasm'));
		await init({ mlkem: mlkemBytes, sha3: sha3Bytes });
	});

	it('mlkem module initialized', () => {
		expect(isInitialized('mlkem')).toBe(true);
	});
...
// Vector: mlkem_keygen.ts[ml_kem_768_keygen[0]]
...
// Vector: mlkem_keygen.ts[ml_kem_512_keygen[0]]

describe('Gate 2, IND-CPA keygen all param sets', () => {
	function runKeygen(vectors: typeof ml_kem_512_keygen, params: MlKemParams) {
...
// Vector: mlkem_encapdecap.ts[ml_kem_512_encap[0]]

describe('Gate 3, Encapsulation all param sets', () => {
	function runEncap(vectors: typeof ml_kem_512_encap, params: MlKemParams) {
...
// Vector: mlkem_encapdecap.ts[ml_kem_512_decap_val[0]]

describe('Gate 4, Decapsulation valid', () => {
	function runDecapValid(vectors: typeof ml_kem_512_decap_val, params: MlKemParams) {
...
describe('Gate 5, Decapsulation implicit rejection', () => {
	function runDecapReject(vectors: typeof ml_kem_512_decap_val, params: MlKemParams) {
...
// Vector: mlkem_encapdecap.ts[ml_kem_512_encap_key_check[0]]

describe('Gate 6, Encapsulation key validation', () => {
	function runEncapKeyCheck(vectors: typeof ml_kem_512_encap_key_check, params: MlKemParams) {
...
// Vector: mlkem_encapdecap.ts[ml_kem_512_decap_key_check[0]]

describe('Gate 7, Decapsulation key validation', () => {
	function runDecapKeyCheck(vectors: typeof ml_kem_512_decap_key_check, params: MlKemParams) {
...
describe('Gate 8, Round-trip property', () => {
	function runRoundTrip(params: MlKemParams, label: string) {
		describe(label, () => {
...
describe('Gate 9, Implicit rejection property', () => {
	function runRejection(params: MlKemParams, label: string) {
		describe(label, () => {
			for (let i = 0; i < 5; i++) {
				it(`iteration ${i}`, () => {
