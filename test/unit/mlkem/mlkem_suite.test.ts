...
//
/**
 * MlKemSuite, hybrid KEM + symmetric AEAD cipher suite tests.
 */
...
for (const [kemName, mkKem, params, kemNibble] of PARAM_SETS) {
	for (const [cipherName, inner, cipherNibble] of INNER_CIPHERS) {
		describe(`MlKemSuite: ${kemName} + ${cipherName}`, () => {
			it('Seal encrypt/decrypt round-trip', () => {
				const kem = mkKem();
				const suite = MlKemSuite(kem, inner);
...
describe('MlKemSuite error cases', () => {
	...
	it('Seal.decrypt with wrong suite throws format mismatch', () => {
		const kem = new MlKem512();
		const suite512x = MlKemSuite(kem, XChaCha20Cipher);
		const suite512s = MlKemSuite(kem, SerpentCipher);
...
describe('MlKemSuite param set key sizes', () => {
