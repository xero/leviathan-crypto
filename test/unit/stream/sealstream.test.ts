	...
	describe(`SealStream / OpenStream — ${name}`, () => {
		...
		describe('round-trip', () => {
			it('single chunk', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, [pt]);
				expect(header.length).toBe(HEADER_SIZE);
				const result = openAll(cipher, key, header, encrypted);
			...
			it('multi-chunk (5 varying sizes)', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, chunks, { chunkSize: 4096 });
				const result = openAll(cipher, key, header, encrypted);
			...
			it('minimum chunk size (1024)', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, [pt], { chunkSize: CHUNK_MIN });
				const result = openAll(cipher, key, header, encrypted);
				expect(result[0]).toEqual(pt);
			});
			...
			it('max configured chunk size', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, [pt], { chunkSize: 8192 });
				const result = openAll(cipher, key, header, encrypted);
				expect(result[0]).toEqual(pt);
			});
		});
		...
		describe('per-chunk AAD', () => {
			it('seal with AAD, open with same AAD', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, [pt], { aads: [aad] });
				const result = openAll(cipher, key, header, encrypted, { aads: [aad] });
				expect(result[0]).toEqual(pt);
			});
			...
			it('AAD mismatch → AuthenticationError', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, [pt], { aads: [aad1] });
				expect(() => openAll(cipher, key, header, encrypted, { aads: [aad2] }))
		...
		describe('framed mode', () => {
			it('round-trip with framed=true', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, chunks, {
					chunkSize: 2048,
					framed: true,
				});
				const result = openAll(cipher, key, header, encrypted);
		...
		describe('authentication', () => {
			it('tampered chunk body → AuthenticationError', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, [pt]);
				encrypted[0][5] ^= 0xff;
				expect(() => openAll(cipher, key, header, encrypted))
					.toThrow(AuthenticationError);
			});
			...
			it('chunk reorder → AuthenticationError', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, chunks, { chunkSize: 2048 });
			...
			it('replay chunk from different stream → AuthenticationError', () => {
				...
				const { header: header2 } = sealAndCollect(cipher, key2, [pt]);
				expect(() => openAll(cipher, key2, header2, encrypted1))
					.toThrow(AuthenticationError);
			});
		...
		describe('seek', () => {
			it('seek to specific chunk', () => {
				...
				const { header, encrypted } = sealAndCollect(cipher, key, chunks, { chunkSize: 1024 });
				const opener = new OpenStream(cipher, key, header);
				opener.seek(5);
		...
		describe('TransformStream', () => {
			...
			it('last chunk gets TAG_FINAL — no extra empty chunk', async () => {
				...
				// header + 2 encrypted chunks (NOT header + 2 + empty final)
		...
		describe('state machine', () => {
			...
			it('separate streams with same key but different nonces', () => {
				...
				const { header: h1, encrypted: e1 } = sealAndCollect(cipher, key, [pt1]);
				const { header: h2, encrypted: e2 } = sealAndCollect(cipher, key, [pt2]);
				// Different nonces — headers differ
				expect(h1).not.toEqual(h2);
				// Both decrypt independently
				expect(openAll(cipher, key, h1, e1)[0]).toEqual(pt1);
				expect(openAll(cipher, key, h2, e2)[0]).toEqual(pt2);
			});
		});
...
describe('cross-cipher rejection', () => {
	it('XChaCha20 header → SerpentCipher opener → throws Error (not AuthenticationError)', () => {
		const key = randomBytes(32);
		const sealer = new SealStream(XChaCha20Cipher, key);
		const header = sealer.header;
		sealer.finalize(new Uint8Array(0));
		expect(() => new OpenStream(SerpentCipher, key, header))
			.toThrow(/expected format 0x02.*got 0x01/);
	});

	it('Serpent header → XChaCha20Cipher opener → throws Error (not AuthenticationError)', () => {
		const key = randomBytes(32);
		const sealer = new SealStream(SerpentCipher, key);
		const header = sealer.header;
		sealer.finalize(new Uint8Array(0));
		expect(() => new OpenStream(XChaCha20Cipher, key, header))
	...
	it('format mismatch error is Error, not AuthenticationError', () => {
		...
		sealer.finalize(new Uint8Array(0));
		try {
			new OpenStream(SerpentCipher, key, header);
...
// ── OpenStream header chunkSize validation ──────────────────────────────────

describe('OpenStream header chunkSize validation', () => {
...
describe('stream layer sha2 init gate', () => {
	...
	it('OpenStream without sha2 → clear error', async () => {
		...
		_resetForTesting();
		await init({ chacha20: chacha20Wasm });
		expect(() => new OpenStream(XChaCha20Cipher, randomBytes(32), header))
			.toThrow(/sha2/);
		// Restore
		await init({ chacha20: chacha20Wasm, serpent: serpentWasm, sha2: sha2Wasm });
	});
...
describe('SerpentCipher specific', () => {
	...
	it('counter binding in HMAC: chunk at counter=3 fails at counter=5', () => {
		...
		const { header, encrypted } = sealAndCollect(SerpentCipher, key2, chunks, { chunkSize: 1024 });
		const opener = new OpenStream(SerpentCipher, key2, header);
