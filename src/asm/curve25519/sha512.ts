...
//                           ▀█████▀▀
//
// src/asm/curve25519/sha512.ts
//
// SHA-512 embedded into curve25519.wasm.
//
// Verbatim port from src/asm/sha2/sha512.ts at commit 3ffe9044873c6b253ca872b9333c8db84327aad1.
// Buffer offsets are local to curve25519's linear-memory layout
// (see ./buffers.ts) but the algorithm code is byte-equivalent
// to the canonical source. To update: replace from sha2/sha512.ts
// and verify via `diff src/asm/sha2/sha512.ts src/asm/curve25519/sha512.ts`
// ignoring buffer-offset import lines.
//
// Rationale for embed-not-orchestrate: per-call WASM lifecycle
// plus 2-4 SHA-512 invocations per Ed25519 sign/verify makes
// sha2.wasm orchestration cost 6-12 JS<->WASM boundary crossings
...
//      only uses SHA-512 (FIPS 180-4 §6.4); the truncated variants are
//      dead code in this module.
//   3. Module-internal `sha512UpdateBytes(src, len)` appended at the
//      bottom: a chunking wrapper around sha512Update for the Ed25519
//      hot path where input pieces (seed, prefix, message, R, pk,
//      digest, dom2 prefix) live at arbitrary memory offsets. Loops
//      memory.copy + sha512Update in 128-byte chunks.
//
// Standard: FIPS 180-4, "Secure Hash Standard", August 2015
// URL: https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.180-4.pdf
// Sections: §4.1.3 (functions), §4.2.3 (constants), §5.3.5 (IV),
//           §6.4 (SHA-512 algorithm)
//
...
//   sha512UpdateBytes(s,n) , chunked sha512Update from arbitrary memory offset
...
// ── Non-verbatim helper appended for curve25519's call pattern ──────────────
//
// sha512UpdateBytes(src, len): chunked sha512Update from an arbitrary memory
// offset. Ed25519 hashes pieces that live in user-provided buffers (seed,
// message, digest) and in module scratch (prefix, R, pk, dom2 prefix) which
// are not pre-staged in SHA512_INPUT_OFFSET. Loops memory.copy into the
// staging buffer + sha512Update in ≤128-byte chunks.

export function sha512UpdateBytes(src: i32, len: i32): void {
	let off:       i32 = src
	let remaining: i32 = len
	while (remaining > 0) {
		const chunk: i32 = remaining > 128 ? 128 : remaining
		memory.copy(SHA512_INPUT_OFFSET, off, chunk)
		sha512Update(chunk)
		off       += chunk
		remaining -= chunk
	}
