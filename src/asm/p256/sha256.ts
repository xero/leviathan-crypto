...
//                           ▀█████▀▀
//
// src/asm/p256/sha256.ts
//
// SHA-256 embedded into p256.wasm.
//
// Verbatim port from src/asm/sha2/sha256.ts. Buffer offsets are local
// to the p256 module's linear-memory layout (see ./buffers.ts) but the
// algorithm code is byte-equivalent to the canonical source. To update:
// replace from sha2/sha256.ts and verify via
// `diff src/asm/sha2/sha256.ts src/asm/p256/sha256.ts` ignoring
// buffer-offset import lines.
//
// Rationale for embed-not-orchestrate: RFC 6979 §3.2 K derivation drives
// many HMAC-SHA-256 calls per sign (each iteration of the rejection
// loop is two HMAC inner-hash compress invocations plus an outer hash;
// expected loop count is 1 but the worst-case bound is unbounded).
// Each cross-WASM-boundary call costs more than the hash itself.
// Embedding keeps the entire signing path inside one WASM call. Mirrors
// curve25519's embedded SHA-512 posture (see head of
// src/asm/curve25519/sha512.ts) where the Ed25519 hot path runs
// SHA-512 inside curve25519.wasm for the same reason.
//
...
//   2. SHA-224 IV constants and entry points (sha224Init / sha224Final)
//      stripped: ECDSA-P256 in FIPS 186-5 + RFC 6979 only uses SHA-256;
//      the truncated variant is dead code in this module.
//   3. Module-internal `sha256UpdateBytes(src, len)` appended at the
//      bottom: a chunking wrapper around sha256Update for the HMAC-SHA-256
//      hot path where input pieces (V, K, octet separators, int2octets(d),
//      bits2octets(h1)) live at arbitrary memory offsets. Loops
//      memory.copy + sha256Update in 64-byte chunks.
//
// Standard: FIPS 180-4, "Secure Hash Standard", August 2015
// URL: https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.180-4.pdf
// Sections: §4.1.2 (functions), §4.2.2 (constants), §5.3.3 (IV),
//           §6.2 (SHA-256 algorithm)
//
// Streaming API (module-internal; NOT re-exported from index.ts):
//   sha256Init()           , initialize SHA-256 IVs + clear partial/total state
//   sha256Update(len)      , process len bytes from SHA256_INPUT_OFFSET (len ≤ 64)
//   sha256Final()          , pad, compress, write 32-byte digest to SHA256_OUT_OFFSET
//   sha256UpdateBytes(s,n) , chunked sha256Update from arbitrary memory offset
...
// ── Chunked update over arbitrary linear-memory source ──────────────────────
//
// HMAC / RFC 6979 call paths assemble inputs from regions outside
// SHA256_INPUT_OFFSET (HMAC_DRBG_K, HMAC_DRBG_V, int2octets(d) and
// bits2octets(h1) staged into the SCALAR_TMP area, octet separator
// bytes inline). This helper hides the 64-byte staging dance: copy
// chunks of up to 64 bytes from `src` into SHA256_INPUT_OFFSET, then
// call sha256Update. Loops over the input until len bytes have been
// consumed. Constant-time wrt len within each fixed-size chunk.

export function sha256UpdateBytes(src: i32, len: i32): void {
	let remaining: i32 = len
	let pos:       i32 = src
	while (remaining > 0) {
		const take: i32 = remaining > 64 ? 64 : remaining
		memory.copy(SHA256_INPUT_OFFSET, pos, take)
		sha256Update(take)
		pos       += take
		remaining -= take
	}
