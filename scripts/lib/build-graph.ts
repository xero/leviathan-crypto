...
export const TARGETS: Record<BuildTarget, TargetDef> = {
	...
	embed: {
		deps: ['asm'],
		run: async () => {
			const toRemove = EMBED_MODULES.map(n => `src/ts/embedded/${n}.ts`).concat(['src/ts/ct-wasm.ts'])
	...
	'embed-workers': {
		...
		// src/ts/ct-wasm.ts which is generated by `embed`. embed-workers therefore
	...
	all: {
		...
		run: async (visited) => {
			...
			// transitively imports src/ts/ct-wasm.ts emitted by embed).
