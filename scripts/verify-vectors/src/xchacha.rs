// XChaCha20 v3 verifier for both seal blobs (single chunk) and sealstream
...
//                           bits 5..0 = format ID. XChaCha20 v3 = 0x03.
//
//   chunk_i:   ChaCha20-Poly1305(subkey, counterNonce_i, plaintext_i, aad="")
...
use crate::primitives::hchacha20::hchacha20;
use crate::parse::{SealVector, SealStreamVector};

const INFO_V3:     &[u8]  = b"xchacha20-sealstream-v3";
const HEADER_SIZE: usize  = 20;
const COMMIT_SIZE: usize  = 32;
const TAG_SIZE:    usize  = 16;
const FORMAT_ENUM: u8     = 0x03;
...
pub fn derive_v3(master_key: &[u8; 32], nonce16: &[u8; 16], header: &[u8]) -> DerivedV3 {
    ...
    let subkey = hchacha20(&stream_key, nonce16);
    DerivedV3 { subkey, commitment }
}
...
fn aead_encrypt(subkey: &[u8; 32], counter: u64, final_flag: u8, plaintext: &[u8]) -> Vec<u8> {
...
pub fn verify_seal(v: &SealVector) -> (bool, Vec<String>) {
    ...
    // Chunk 0, TAG_FINAL.
    let aead_out = aead_encrypt(&check.derived.subkey, 0, TAG_FINAL, &v.plaintext);
    ...
    log.push("  ✓ blob matches preamble ‖ ChaCha20-Poly1305(subkey, counter=0|FINAL, pt, aad=∅)".to_string());
...
pub fn verify_sealstream(v: &SealStreamVector) -> (bool, Vec<String>) {
    ...
    for (i, chunk) in v.chunks.iter().enumerate() {
        ...
        let aead_out = aead_encrypt(&check.derived.subkey, counter, final_flag, &chunk.plaintext);
